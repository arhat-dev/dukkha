...
func parseDir(path string) map[string]*ast.Package {
	...
	if err != nil {
		panic(fmt.Errorf("At least one error when parsing directory: %w", err))
	}
	return dir
}
...
func ImportPathFromSelector(it *ast.SelectorExpr) (path string, name string, err error) {
	...
	if importSpec.Path.Kind != token.STRING {
		return "", "", fmt.Errorf("Cannot handle token of type %s as import path", importSpec.Path.Kind)
	}
	// Trim surrounding quotes
	importPath, err := strconv.Unquote(importSpec.Path.Value)
	if err != nil {
		panic(fmt.Errorf("Impossible! string not quoted: %s", importSpec.Path.Value))
	}
...
func (importer Importer) SearchEntryPackageForObj(typeName string) (*ast.Object, bool) {
	...
	if err != nil {
		panic(fmt.Errorf("Error importing starting package! %w", err))
	}
...
func (importer Importer) findImportedObj(it *ast.SelectorExpr) (*ast.Object, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("couldn't get import path: %w", err)
	}

	importedPkg, err := importer(importPath)
	if err != nil {
		return nil, fmt.Errorf("couldn't handle struct field: %w", err)
	}
	...
	obj, ok := scope.Objects[typeName]
	if !ok {
		return nil, fmt.Errorf("Couldn't find object %s in imported package %s", it.Sel.Name, importPath)
	}
	return obj, nil
}
...
func (importer Importer) FindImportedTypeSpec(it *ast.SelectorExpr) (importPath, typeName string, _ *ast.TypeSpec, _ error) {
	...
	if !ok {
		return "", "", nil, fmt.Errorf(
			"Expected TypeSpec for %s.%s", importPath, it.Sel.Name,
		)
	}
	return importPath, it.Sel.Name, typeSpec, nil
}
