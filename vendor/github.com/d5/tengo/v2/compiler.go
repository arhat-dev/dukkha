...
type Compiler[S SymbolTableStorage[S], M ModuleGetter[M]] struct {
...
func NewCompiler[S SymbolTableStorage[S], M ModuleGetter[M]](
	file *parser.SourceFile,
	symbolTable *SymbolTable[S],
	constants []Object,
	modules M,
	...
	if symbolTable == nil {
		var _tmp S
		symbolTable = NewSymbolTable(_tmp.New())
	}

	// add builtin functions to the symbol table
	symbolTable.BatchDefineAllBuiltin(builtinFunctionSymbols[:])
...
func (c *Compiler[S, M]) Compile(node parser.Node) error {
	if c.trace != nil {
		if node != nil {
	...
	case *parser.FuncLit:
		...
		for _, p := range node.Type.Params.List {
			s := Symbol{
				Name:          p.Name,
				LocalAssigned: true, // function arguments is not assigned directly.
			}
			c.symbolTable.Define(&s)
		}
		...
		freeSymbols := c.symbolTable.GetFreeSymbols()
		numLocals := c.symbolTable.GetMaxSymbols()
...
func (c *Compiler[S, M]) Bytecode() *Bytecode {
	return &Bytecode{
		FileSet: c.file.Set(),
...
func (c *Compiler[S, M]) EnableFileImport(enable bool) {
	c.allowFileImport = enable
}

// SetImportDir sets the initial import directory path for file imports.
func (c *Compiler[S, M]) SetImportDir(dir string) {
	c.importDir = dir
}
...
func (c *Compiler[S, M]) SetImportFileExt(exts ...string) error {
	if len(exts) == 0 {
...
// local module files.
func (c *Compiler[S, M]) GetImportFileExt() []string {
	return c.importFileExt
}

func (c *Compiler[S, M]) compileAssign(
	node parser.Node,
	lhs, rhs []parser.Expr,
	op token.Token,
) error {
...
func (c *Compiler[S, M]) compileLogical(node *parser.BinaryExpr) error {
	// left side term
	if err := c.Compile(node.LHS); err != nil {
		return err
	}
...
func (c *Compiler[S, M]) compileForStmt(stmt *parser.ForStmt) error {
	c.symbolTable = c.symbolTable.Fork(true)
	defer func() {
		c.symbolTable = c.symbolTable.Parent(false)
	}()
...
func (c *Compiler[S, M]) compileForInStmt(stmt *parser.ForInStmt) error {
	c.symbolTable = c.symbolTable.Fork(true)
	defer func() {
		c.symbolTable = c.symbolTable.Parent(false)
	}()
	...
	if stmt.Key.Name != "_" {
		...
		c.symbolTable.Define(&keySymbol)
		if itSymbol.Scope == ScopeGlobal {
			c.emit(stmt, parser.OpGetGlobal, itSymbol.Index)
		} else {
			c.emit(stmt, parser.OpGetLocal, itSymbol.Index)
	...
	if stmt.Value.Name != "_" {
		...
		c.symbolTable.Define(&valueSymbol)
		if itSymbol.Scope == ScopeGlobal {
			c.emit(stmt, parser.OpGetGlobal, itSymbol.Index)
		} else {
			c.emit(stmt, parser.OpGetLocal, itSymbol.Index)
...
func (c *Compiler[S, M]) checkCyclicImports(
	node parser.Node,
	modulePath string,
) error {
...
func (c *Compiler[S, M]) compileModule(
	node parser.Node,
	modulePath string,
	src []byte,
	isFile bool,
	...
	var _tmp S
	symbolTable := NewSymbolTable(_tmp.New())
	symbolTable.BatchDefineAllBuiltin(c.symbolTable.GetBuiltinSymbols())
	...
	compiledFunc.NumLocals = symbolTable.GetMaxSymbols()
	c.storeCompiledModule(modulePath, compiledFunc)
	return compiledFunc, nil
}

func (c *Compiler[S, M]) loadCompiledModule(
	modulePath string,
...
func (c *Compiler[S, M]) storeCompiledModule(
	modulePath string,
...
func (c *Compiler[S, M]) enterLoop() *loop {
	loop := &loop{}
...
func (c *Compiler[S, M]) currentLoop() *loop {
	if c.loopIndex >= 0 {
...
func (c *Compiler[S, M]) currentInstructions() []byte {
	return c.scopes[c.scopeIndex].Instructions
}

func (c *Compiler[S, M]) currentSourceMap() map[int]parser.Pos {
	return c.scopes[c.scopeIndex].SourceMap
}

func (c *Compiler[S, M]) enterScope() {
...
func (c *Compiler[S, M]) leaveScope() (
	instructions []byte,
	sourceMap map[int]parser.Pos,
) {
...
func (c *Compiler[S, M]) error(node parser.Node, err error) error {
...
func (c *Compiler[S, M]) addConstant(o Object) int {
	if c.parent != nil {
...
func (c *Compiler[S, M]) addInstruction(b []byte) int {
...
func (c *Compiler[S, M]) replaceInstruction(pos int, inst []byte) {
...
func (c *Compiler[S, M]) changeOperand(opPos int, operand ...int) {
...
func (c *Compiler[S, M]) optimizeFunc(node parser.Node) {
...
func (c *Compiler[S, M]) printTrace(a ...interface{}) {
	const (
		dots = ". . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . "
		n    = len(dots)
	)
...
func (c *Compiler[S, M]) getPathModule(moduleName string) (pathFile string, err error) {
...
func tracec[S SymbolTableStorage[S], M ModuleGetter[M]](c *Compiler[S, M], msg string) *Compiler[S, M] {
	c.printTrace(msg, "{")
	c.indent++
	return c
}

func untracec[S SymbolTableStorage[S], M ModuleGetter[M]](c *Compiler[S, M]) {
	c.indent--
	c.printTrace("}")
}
