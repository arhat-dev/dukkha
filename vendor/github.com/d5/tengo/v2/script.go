...
type Script[S SymbolTableStorage[S], M ModuleGetter[M]] struct {
...
func NewScript[S SymbolTableStorage[S], M ModuleGetter[M]](input []byte) *Script[S, M] {
	return &Script[S, M]{
...
func (s *Script[S, M]) Add(name string, value interface{}) error {
...
func (s *Script[S, M]) Remove(name string) bool {
...
func (s *Script[S, M]) SetImports(modules M) {
	s.modules = modules
}

// SetImportDir sets the initial import directory for script files.
func (s *Script[S, M]) SetImportDir(dir string) error {
...
func (s *Script[S, M]) Compile() (*Compiled, error) {
	...
	// reduce globals size
	globals = globals[:symbolTable.GetMaxSymbols()+1]
...
// global variables.
func (s *Script[S, M]) Run() (compiled *Compiled, err error) {
...
func (s *Script[S, M]) prepCompile() (
	...
	for idx, name := range names {
		...
		globals[sym.Index] = s.variables[name].value
	}
	return
}
