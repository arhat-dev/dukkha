...
func TestResolveActions_steps(t *testing.T) {
	...
	testhelper.TestFixtures(t, "./_fixtures/actions",
		...
		func(t *testing.T, spec, exp interface{}) {
			...
			i := 0
			for jobs != nil {
				assertActionsVisibleFields(t, i, cs.Steps[i].Actions, in.Actions)
				i++
...
func assertActionsVisibleFields(t *testing.T, round int, ex, ac []*Action) {
	roundX := fmt.Sprintf("at round #%d", round)
	for i := range ex {
		expected, actual := ex[i], ac[i]

		assert.EqualValues(t, expected.Name, actual.Name, roundX)
		assert.EqualValues(t, expected.Env, actual.Env, roundX)
		assert.EqualValues(t, expected.Chdir, actual.Chdir, roundX)
		assert.EqualValues(t, expected.ContinueOnError, actual.ContinueOnError, roundX)
		assert.EqualValues(t, expected.Cmd, actual.Cmd, roundX)
		assert.EqualValues(t, expected.Idle, actual.Idle, roundX)
		assert.EqualValues(t, expected.Task, actual.Task, roundX)
		assert.EqualValues(t, expected.EmbeddedShell, actual.EmbeddedShell, roundX)
		assert.EqualValues(t, expected.ExternalShell, actual.ExternalShell, roundX)
	}
}
